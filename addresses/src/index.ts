 ...
 * Multicall3 `0xca11…` < Chainlink stETH/USD feed `0xcfe5…`. Then the
 * AttestationRegistry rungs by chainId: Base mainnet (8453) < local Anvil
 * (31337) < Base Sepolia (84532). The two Base rungs share one deployed
 * address; Anvil's is the standard account-0 CREATE.
 */
export const ADDRESS_TABLE: readonly AddressEntry[] = [
  ...
  // The AttestationRegistry anchor (AD-14), one entry per deploy-ladder rung.
  // Each `address` is transcribed lowercase from that chain's
  // `.transactions[].contractAddress` in
  // `packages/contracts/broadcast/DeployAttestationRegistry.s.sol/<chainId>/run-latest.json`
  // (gitignored), and each `checksummed` is transcribed INDEPENDENTLY from
  // `cast to-check-sum-address` / Basescan — never by lowercasing (SEC-1). The two
  // Base rungs (8453, 84532) share one address: identical `CREATE(deployer, nonce
  // 0)` on both chains; Anvil's (31337) is the standard account-0 CREATE.

  // Base mainnet (8453) — the single canonical anchor, deployed + Basescan-verified
  // in Batch 6 (Story 3.3). Its attest tx + verified-source URL are recorded in
  // `packages/contracts/README.md` Deployments. This entry is what wakes the web's
  // anchor panel (default `VITE_ANCHOR_CHAIN_ID=8453`) from "not yet anchored".
  {
    chainId: 8453,
    kind: "registry",
    symbol: "AttestationRegistry",
    address: "0xedb4a12ce8bd024cd3f1820ded1b1afd7f5b3dce",
    checksummed: "0xeDb4a12cE8bd024Cd3f1820DEd1B1AFD7F5B3Dce",
  },
  // Local Anvil (31337) — the deterministic first CREATE from Anvil account 0 at
  // nonce 0, a toolchain constant (not a project identity). Source of truth: the
  // `31337/run-latest.json` contractAddress (gitignored — regenerated by the
  // local deploy).
  {
    chainId: 31337,
    kind: "registry",
  ...
  // Base Sepolia (84532) — the deploy-ladder REHEARSAL rung, deployed + Basescan-
  // verified in Batch 6. Same address as the 8453 mainnet rung (above); its
  // verified-source URL is in `packages/contracts/README.md` Deployments.
  {
    chainId: 84532,
    kind: "registry",
    symbol: "AttestationRegistry",
