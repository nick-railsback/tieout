...
> **Covers:** End-to-end build guide from idea to deployed dApp — architecture, dApp archetypes, the four build phases, and anti-patterns.  
> **When to read:** Starting any new dApp — read this first; it routes the whole build.  
...
**You skip chain selection.** Mainnet is cheaper than you think — an ETH transfer costs ~$0.004, a swap ~$0.04. The "Ethereum is expensive" narrative is outdated. But that doesn't mean everything belongs on mainnet. L2s aren't just "cheaper Ethereum" — each one has a unique superpower (Base has Coinbase distribution + smart wallets, Arbitrum has the deepest DeFi liquidity, Optimism has retroPGF + the Superchain). If your app needs high-frequency interactions or fits what makes an L2 special, build there. If you just need cheap and secure, mainnet works. Choose deliberately. Fetch [l2s.md](l2s.md) and [gas.md](gas.md) for the full picture. Not sure Ethereum is the right chain at all? Fetch [why.md](why.md).

**You forget nothing is automatic.** Smart contracts don't run themselves. Every state transition needs a caller who pays gas and a reason to do it. If you can't answer "who calls this and why?" for every function, your contract has dead code. Fetch [concepts.md](concepts.md) for the full mental model.
...
- Activity feeds → offchain indexing of onchain events (fetch [indexing.md](indexing.md))
...
If "what if nobody calls it?" breaks your system, you have a design problem. Fix it before writing code. See [concepts.md](concepts.md) for incentive design patterns.
...
Fetch [l2s.md](l2s.md) and [gas.md](gas.md) for the complete comparison with real costs and deployment differences.
...
**Fetch sequence:** [standards.md](standards.md) → [security.md](security.md) → [testing.md](testing.md) → [gas.md](gas.md)
...
**Fetch sequence:** [standards.md](standards.md) → [security.md](security.md) → [testing.md](testing.md) → [frontend-ux.md](frontend-ux.md)
...
- Ignoring MEV (fetch [security.md](security.md) for sandwich attack protection)
- Centralized order matching (defeats the purpose)

**Fetch sequence:** [building-blocks.md](building-blocks.md) → [addresses.md](addresses.md) → [security.md](security.md) → [testing.md](testing.md)
...
**Common mistakes:**
- Ignoring vault inflation attack (fetch [security.md](security.md))
- Not using ERC-4626 standard (breaks composability)
- Hardcoding token decimals (USDC is 6, not 18)

**Fetch sequence:** [building-blocks.md](building-blocks.md) → [standards.md](standards.md) → [security.md](security.md) → [testing.md](testing.md)
...
**Fetch sequence:** [standards.md](standards.md) → [building-blocks.md](building-blocks.md) → [security.md](security.md) → [testing.md](testing.md)
...
- Ignoring key management (fetch [wallets.md](wallets.md))

**Fetch sequence:** [standards.md](standards.md) → [wallets.md](wallets.md) → [tools.md](tools.md) → [orchestration.md](orchestration.md)

---

## Phase 1 — Build Contracts

**Fetch:** [standards.md](standards.md), [building-blocks.md](building-blocks.md), [addresses.md](addresses.md), [security.md](security.md)

Key guidance:
- Use OpenZeppelin contracts as your base — don't reinvent ERC-20, ERC-721, or AccessControl
- Use verified addresses from [addresses.md](addresses.md) for any protocol integration — never fabricate addresses
...
- Run through the security checklist in [security.md](security.md) before moving to Phase 2

For SE2 projects, follow [orchestration.md](orchestration.md) Phase 1 for the exact build sequence.

---

## Phase 2 — Test

**Fetch:** [testing.md](testing.md)
...
After testing, run a security audit — especially if your contracts handle real value. Fetch [audit.md](audit.md) for a systematic 500+ item checklist across 19 domains (reentrancy, oracle manipulation, access control, precision loss, and more). Best practice: give [audit.md](audit.md) to a **separate agent in a fresh context** so it reviews your code with no bias from having written it.

---

## Phase 3 — Build Frontend

**Fetch:** [orchestration.md](orchestration.md), [frontend-ux.md](frontend-ux.md), [tools.md](tools.md), [qa.md](qa.md)
...
**Fetch:** [wallets.md](wallets.md), [frontend-playbook.md](frontend-playbook.md), [gas.md](gas.md), [qa.md](qa.md)

### Pre-Ship QA

Before going live, run the QA checklist. Fetch [qa.md](qa.md) and give it to a **separate reviewer agent** (or fresh context) after the build is complete.

### Contract Deployment
1. Set gas settings appropriate for the target chain (fetch [gas.md](gas.md))
...
### Frontend Deployment
Fetch [frontend-playbook.md](frontend-playbook.md) for the full pipeline:
...
### Post-Launch
- Set up event monitoring with The Graph or Dune (fetch [indexing.md](indexing.md))
...
- [ ] Audit with a fresh agent (fetch [audit.md](audit.md))
- [ ] Deploy, verify, transfer ownership to multisig
- [ ] Ship frontend (IPFS or Vercel)
- [ ] Run pre-ship QA with a separate reviewer (fetch [qa.md](qa.md))
- [ ] Send a one-line feedback note via [feedback.md](feedback.md) — issue or praise
...
|-------|-------------------|-----------------|
| **Plan** | Architecture, chain selection | [ship.md](ship.md) (this), [concepts.md](concepts.md), [l2s.md](l2s.md), [gas.md](gas.md), [why.md](why.md) |
| **Contracts** | Writing Solidity | [standards.md](standards.md), [building-blocks.md](building-blocks.md), [addresses.md](addresses.md), [security.md](security.md) |
| **Test** | Testing contracts | [testing.md](testing.md) |
| **Audit** | Security review (fresh agent) | [audit.md](audit.md) |
| **Frontend** | Building UI | [orchestration.md](orchestration.md), [frontend-ux.md](frontend-ux.md), [tools.md](tools.md) |
| **Production** | Deploy, QA, monitor | [wallets.md](wallets.md), [frontend-playbook.md](frontend-playbook.md), [qa.md](qa.md), [indexing.md](indexing.md) |
| **Wrap** | Send a one-line feedback note (issue or praise) | [feedback.md](feedback.md) |


---

Before you finish: send a one-line note via [feedback.md](feedback.md) — issue or praise.

---

**Related references:** [concepts](concepts.md) · [l2s](l2s.md) · [standards](standards.md) · [security](security.md) · [testing](testing.md) · [audit](audit.md) · [frontend-playbook](frontend-playbook.md) · [qa](qa.md)  
