...
> **Covers:** The build-to-production frontend pipeline — fork-network dev, IPFS deploy footguns, clean builds.  
> **When to read:** Deploying a dApp frontend to production.  
> **Pack index:** [↩ all references](../SKILL.md)


## What You Probably Got Wrong

**"I'll use `yarn chain`."** Wrong. `yarn chain` gives you an empty local chain with no protocols, no tokens, no state. `yarn fork --network base` gives you a copy of real Base with Uniswap, Aave, USDC, real whale balances — everything (verified addresses: [addresses.md](addresses.md)). Always fork.
...
**Related references:** [orchestration](orchestration.md) · [frontend-ux](frontend-ux.md) · [gas](gas.md) · [wallets](wallets.md)  
